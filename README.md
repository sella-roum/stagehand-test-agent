...
- **インタラクティブなシナリオ記録**: 対話モードで `record` と入力すると、記録モードが開始されます。ユーザーが「ログインボタンをクリック」のように指示を出すと、エージェントがその操作を実行。成功した操作の履歴から、最終的に自然言語のテストシナリオファイルを自動生成します。
...
### モード4: シナリオ記録モード (対話的シナリオ生成)

対話モード（確認モードまたは自律的対話モード）の実行中に、テストシナリオをインタラクティブに記録・生成できます。

1.  まず、対話モードのいずれかでエージェントを起動します。
    ```bash
    npm run interactive
    ```
2.  テストシナリオの入力を求められたら、`record`と入力してEnterキーを押します。
    ```
    テストシナリオを入力してください (終了: 'exit', Inspector: 'inspector', 記録開始: 'record'):
    > record
    ```
3.  記録モードが開始されます。ブラウザで行いたい操作を一つずつ自然言語で指示してください。
    ```
    ⏺️ 記録モードを開始します。操作指示を入力してください。(保存: 'save', 中止: 'cancel')
    次の操作指示 > Googleを開いて
    ```
4.  エージェントが指示を実行し、成功すると次の指示を待ちます。操作や検証を繰り返してください。
5.  すべての操作が終わったら、`save`と入力します。記録された操作履歴から自然言語のシナリオが自動生成され、`tests/scenarios/`ディレクトリにファイルとして保存されます。
    ```
    次の操作指示 > save
    ✅ シナリオを tests/scenarios/recorded-2025-09-15T10-30-00-000Z.txt に保存しました。
    ```
6.  記録を途中でやめたい場合は`cancel`と入力します。
